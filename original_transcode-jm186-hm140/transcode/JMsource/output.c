...
static void get_out_picture(VideoParameters *p_Vid, StorablePicture *p, int p_out,unsigned char *tempY,unsigned char *tempU,unsigned char *tempV);//zhulinwei
...
void get_picture(VideoParameters *p_Vid, StorablePicture *p, int p_out, int real_structure,unsigned char *tempY,unsigned char *tempU,unsigned char *tempV)
{
	get_out_picture(p_Vid, p, p_out,tempY,tempU,tempV);
}
...
static void get_out_picture(VideoParameters *p_Vid, StorablePicture *p, int p_out,unsigned char *tempY,unsigned char *tempU,unsigned char *tempV)
{
	InputParameters *p_Inp = p_Vid->p_Inp;
	DecodedPicList *pDecPic;
	...
	p_Vid->img2buf (p->imgY, buf, p->size_x, p->size_y, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom, pDecPic->iYBufStride);
	memcpy(tempY,buf,sizeof(unsigned char)*iLumaSize);//==HEVC-2013-04-12
	if(p_out >=0)
	{
		ret = write(p_out, buf, (p->size_y-crop_bottom-crop_top)*(p->size_x-crop_right-crop_left)*symbol_size_in_bytes);
		if (ret != ((p->size_y-crop_bottom-crop_top)*(p->size_x-crop_right-crop_left)*symbol_size_in_bytes))
	...
	if (p->chroma_format_idc!=YUV400)
	{
		...
		buf = (pDecPic->bValid==1)? pDecPic->pU : pDecPic->pU + iChromaSizeX*symbol_size_in_bytes;
		p_Vid->img2buf (p->imgUV[0], buf, p->size_x_cr, p->size_y_cr, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom, pDecPic->iUVBufStride);
		memcpy(tempU,buf,sizeof(unsigned char)*iChromaSizeX*iChromaSizeY);//==HEVC-2013-04-12
		if(p_out >= 0)
		{
			ret = write(p_out, buf, (p->size_y_cr-crop_bottom-crop_top)*(p->size_x_cr-crop_right-crop_left)* symbol_size_in_bytes);
			if (ret != ((p->size_y_cr-crop_bottom-crop_top)*(p->size_x_cr-crop_right-crop_left)* symbol_size_in_bytes))
		...
		if (!rgb_output)
		{
			buf = (pDecPic->bValid==1)? pDecPic->pV : pDecPic->pV + iChromaSizeX*symbol_size_in_bytes;
			p_Vid->img2buf (p->imgUV[1], buf, p->size_x_cr, p->size_y_cr, symbol_size_in_bytes, crop_left, crop_right, crop_top, crop_bottom, pDecPic->iUVBufStride);
			memcpy(tempV,buf,sizeof(unsigned char)*iChromaSizeX*iChromaSizeY);//==HEVC-2013-04-12
			if(p_out >= 0)
			{
				ret = write(p_out, buf, (p->size_y_cr-crop_bottom-crop_top)*(p->size_x_cr-crop_right-crop_left)*symbol_size_in_bytes);
				if (ret != ((p->size_y_cr-crop_bottom-crop_top)*(p->size_x_cr-crop_right-crop_left)*symbol_size_in_bytes))
...
void get_unpaired_field(VideoParameters *p_Vid, FrameStore* fs, int p_out,unsigned char *tempY,unsigned char *tempU,unsigned char *tempV)
{
	StorablePicture *p;
	assert (fs->is_used<3);
	...
	if(fs->is_used & 0x01)
	{
		...
		fs->frame->view_id = fs->view_id;
#endif
		get_picture (p_Vid, fs->frame, p_out, TOP_FIELD,tempY,tempU,tempV);
	}
	...
	if(fs->is_used & 0x02)
	{
		...
		fs->frame->view_id = fs->view_id;
#endif
		get_picture (p_Vid, fs->frame, p_out, BOTTOM_FIELD,tempY,tempU,tempV);
	}
...
void get_direct_output(VideoParameters *p_Vid, int p_out,unsigned char *tempY,unsigned char *tempU,unsigned char *tempV)
{
	get_unpaired_field(p_Vid, p_Vid->out_buffer, p_out,tempY,tempU,tempV);
...
void get_stored_frame(VideoParameters *p_Vid, FrameStore *fs, int p_out,unsigned char *tempY,unsigned char *tempU,unsigned char *tempV)
{
	// make sure no direct output field is pending
	get_direct_output(p_Vid, p_out,tempY,tempU,tempV);

	if (fs->is_used<3)
	{
		get_unpaired_field(p_Vid, fs, p_out,tempY,tempU,tempV);
	}
	else
	{
		if (fs->recovery_frame)
			p_Vid->recovery_flag = 1;
		if ((!p_Vid->non_conforming_stream) || p_Vid->recovery_flag)
			get_picture(p_Vid, fs->frame, p_out, FRAME,tempY,tempU,tempV);
	}
