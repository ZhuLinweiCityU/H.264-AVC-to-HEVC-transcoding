...
void decode_slice(Slice *currSlice, int current_header,CvMat *Feature0,CvMat *Feature1,CvMat *Feature2,int indexframe)
{
  ...
  if ((current_header == SOP || current_header == SOS) && currSlice->ei_flag == 0)
    decode_one_slice(currSlice,Feature0,Feature1,Feature2,indexframe);
 ...
 ***********************************************************************
 */
int decode_one_frame(DecoderParams *pDecoder,CvMat *Feature0,CvMat *Feature1,CvMat *Feature2,int indexframe)
{
...

  {
    for(iSliceNo=0; iSliceNo<p_Vid->iSliceNumOfCurrPic; iSliceNo++)
    {
      ...
      decode_slice(currSlice, current_header,Feature0,Feature1,Feature2,indexframe);
 ...
 ************************************************************************
 */
void decode_one_slice(Slice *currSlice,CvMat *Feature0,CvMat *Feature1,CvMat *Feature2,int indexframe)
{
  VideoParameters *p_Vid = currSlice->p_Vid;
  Boolean end_of_slice = FALSE;
  Macroblock *currMB = NULL;
  int n0=0,n1=0,n2=0;//Transcoder
  int i,j;//transcoder
  int WIDTH  = p_Vid->width/16;//transcoder
  int HEIGHT = p_Vid->height/16;//transcoder
  int p,q;int W0,W1,W2,H0,H1,H2;

  double *data01 = (double*)malloc(sizeof(double)*WIDTH*HEIGHT);
  double *data02 = (double*)malloc(sizeof(double)*WIDTH*HEIGHT);
  double *data03 = (double*)malloc(sizeof(double)*WIDTH*HEIGHT);
  double *data04 = (double*)malloc(sizeof(double)*WIDTH*HEIGHT);
  double *data05 = (double*)malloc(sizeof(double)*WIDTH*HEIGHT);
  double *data06 = (double*)malloc(sizeof(double)*WIDTH*HEIGHT);
  double *data07 = (double*)malloc(sizeof(double)*WIDTH*HEIGHT);

  double *data11=(double*)malloc(sizeof(double)*WIDTH/2*HEIGHT/2);
  double *data12=(double*)malloc(sizeof(double)*WIDTH/2*HEIGHT/2);
  double *data13=(double*)malloc(sizeof(double)*WIDTH/2*HEIGHT/2);
  double *data14=(double*)malloc(sizeof(double)*WIDTH/2*HEIGHT/2);
  double *data15=(double*)malloc(sizeof(double)*WIDTH/2*HEIGHT/2);
  double *data16=(double*)malloc(sizeof(double)*WIDTH/2*HEIGHT/2);
  double *data17=(double*)malloc(sizeof(double)*WIDTH/2*HEIGHT/2);


  int k=0;int l=0;int n=0;
  double mean=0,var=0;int indexX=0,indexY=0;
  double mean1=0,var1=0;


  double temp1=0,temp2=0,temp3=0,temp4=0,temp5=0,temp6=0,temp7=0,temp8=0;
  int nonzerocoeffY=0;double energycoeffY=0;
  int block_x,block_y;int framesize0,framesize1,framesize2;
  if(p_Vid->width%64==0)  W0 = p_Vid->width/64;
  else                    W0 = p_Vid->width/64+1;

  if(p_Vid->height%64==0) H0 = p_Vid->height/64;
  else                    H0 = p_Vid->height/64+1;
  framesize0 = W0*H0;
  framesize1 = 4*framesize0;
  framesize2 = 4*framesize1;
  ...
  while (end_of_slice == FALSE) // loop over macroblocks
  {
	...
	if (indexframe>=0)
	{
		data01[currMB->mb.y*WIDTH+currMB->mb.x]=currMB->cbp;
		data02[currMB->mb.y*WIDTH+currMB->mb.x]=currMB->mb_type;
		data03[currMB->mb.y*WIDTH+currMB->mb.x]=nonzerocoeffY;
		data04[currMB->mb.y*WIDTH+currMB->mb.x]=energycoeffY;
	}
  }
  //reset_ec_flags(p_Vid);
  //===========================相关特征提取=========================================
  ...
  if (indexframe>=0)
  {
	  for (i=0;i<HEIGHT;i++)
		  for(j=0;j<WIDTH;j++)
	  {
		  temp1=data01[i*WIDTH+j];
		  temp2=data02[i*WIDTH+j];
		  temp3=data03[i*WIDTH+j];
		  temp4=data04[i*WIDTH+j];
		  //------------------------------
		  cvmSet(Feature2,n2+indexframe*framesize2,1,temp1);
		  cvmSet(Feature2,n2+indexframe*framesize2,2,temp2);
		  cvmSet(Feature2,n2+indexframe*framesize2,3,temp3);
		  cvmSet(Feature2,n2+indexframe*framesize2,4,temp4);
		  n2++;
	  }
  }
  //-----------------------------对于32X32-------------------------------------------
  if (indexframe>=0)
  {
	  for (i=0;i<HEIGHT;i+=2)
		  for(j=0;j<WIDTH;j+=2)
		  {
			  ...
			  //-------------------------------------------------------
			  temp1=data01[p*WIDTH+q]+data01[(p+1)*WIDTH+q]+
				  data01[p*WIDTH+(q+1)]+data01[(p+1)*WIDTH+(q+1)];
			  temp2=data02[p*WIDTH+q]+data02[(p+1)*WIDTH+q]+
				  data02[p*WIDTH+(q+1)]+data02[(p+1)*WIDTH+(q+1)];
			  temp3=data03[p*WIDTH+q]+data03[(p+1)*WIDTH+q]+
				  data03[p*WIDTH+(q+1)]+data03[(p+1)*WIDTH+(q+1)];
			  temp4=data04[p*WIDTH+q]+data04[(p+1)*WIDTH+q]+
				  data04[p*WIDTH+(q+1)]+data04[(p+1)*WIDTH+(q+1)];

			  data11[p/2*WIDTH/2+q/2]=temp1;
			  data12[p/2*WIDTH/2+q/2]=temp2;
			  data13[p/2*WIDTH/2+q/2]=temp3;
			  data14[p/2*WIDTH/2+q/2]=temp4;
			  //----------------------------------------------------------------------------
			  cvmSet(Feature1,n1+indexframe*framesize1,1,temp1);
			  cvmSet(Feature1,n1+indexframe*framesize1,2,temp2);
			  cvmSet(Feature1,n1+indexframe*framesize1,3,temp3);
			  cvmSet(Feature1,n1+indexframe*framesize1,4,temp4);
			  //-------------------------------------------------------
			  n1++;
		  }
  }
  ...
  if (indexframe>=0)
  {
	  for (i=0;i<HEIGHT/2;i+=2)
		  ...
		  {
			  p=i;
			  if(p>=HEIGHT/2-1) p=HEIGHT/2-2;

			  q=j;
			  if(q>=WIDTH/2-1) q=WIDTH/2-2;
			  //-------------------------------------------------------
			  temp1=data11[p*(WIDTH/2)+q]+data11[(p+1)*(WIDTH/2)+q]+
				  data11[p*(WIDTH/2)+(q+1)]+data11[(p+1)*(WIDTH/2)+(q+1)];
			  temp2=data12[p*(WIDTH/2)+q]+data12[(p+1)*(WIDTH/2)+q]+
				  data12[p*(WIDTH/2)+(q+1)]+data12[(p+1)*(WIDTH/2)+(q+1)];
			  temp3=data13[p*(WIDTH/2)+q]+data13[(p+1)*(WIDTH/2)+q]+
				  data13[p*(WIDTH/2)+(q+1)]+data13[(p+1)*(WIDTH/2)+(q+1)];
			  temp4=data14[p*(WIDTH/2)+q]+data14[(p+1)*(WIDTH/2)+q]+
				  data14[p*(WIDTH/2)+(q+1)]+data14[(p+1)*(WIDTH/2)+(q+1)];
			  //---------------------------------------------------------------------------------
			  cvmSet(Feature0,n0+indexframe*framesize0,1,temp1);
			  cvmSet(Feature0,n0+indexframe*framesize0,2,temp2);
			  cvmSet(Feature0,n0+indexframe*framesize0,3,temp3);
			  cvmSet(Feature0,n0+indexframe*framesize0,4,temp4);
			  //---------------------------------------------------------------------
			  n0++;
		  }
  }
  free(data01);free(data02);free(data03);free(data04);free(data05);free(data06);free(data07);
  free(data11);free(data12);free(data13);free(data14);free(data15);free(data16);free(data17);
  //-----------------------------------------------------------------------------------------
}
